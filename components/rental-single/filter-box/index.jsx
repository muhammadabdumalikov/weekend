...
const index = ({ rental }) => {
  ...
  const handlePhoneClick = (phone, e) => {
    ...
    const greetingMessage = encodeURIComponent(`${t("contact.greeting")} ${rental?.title || ''}`);
  ...
  return (
    <>
      ...
      {showContactPhones && rental?.contact_phone && rental.contact_phone.length > 0 && (
        <div className="col-12">
          <div className="border-top-light pt-20">
            <h5 className="text-16 fw-500 mb-15">{t("contact.contactInfo")}</h5>
            <div className="row y-gap-10">
              {rental.contact_phone.map((phone, index) => (
